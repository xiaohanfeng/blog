...
{
    $model = Db::table('admin_config');
    $map['name'] = $name;
    return $model->where($map)->value('value');
}
...
if (!function_exists('getFieldById')) {
    ...
    /**
     * 根据编号获取名称
     * @param int $id 编号 $table 表名 $name字段名 $id_name编号字段名
     * @return string
     */
    function getFieldById($id, $table, $name = 'name', $id_name = 'id')
    {

        $model = Db::table($table);
        $map[$id_name] = $id;
        return $model->where($map)->value($name);
    }
...
if (!function_exists('arrayToOne')) {
     ...
     * @return string
     */
    function arrayToOne($array, $name)
    {
        $newarray = array();
        if (!empty($array)) {
